...
# if you control the language well, this is not necessary.
...
```shell
SQL>
ld_dir('/{data_dir}/freebase/FilterFreebase', 'FilterFreebase', 'http://freebase.com'); 
rdf_loader_run(); 
```

```shell
SQL>
ld_dir('/{data_dir}/freebase', 'fbwq.nt', 'http://freebase.com'); 
rdf_loader_run(); 
```
...
```shell
SQL>
SELECT ll_state, COUNT(ll_state) FROM load_list GROUP BY ll_state
sparql PREFIX ns: <http://rdf.freebase.com/ns/> SELECT (COUNT(*) as ?Triples) WHERE { ?s ?p ?o .}
```
