...
if __name__ == '__main__':
    ...
    parser.add_argument("--output_file", type=str,
                        default=r"/ropkg/ToG_on_public_kg/ToG_0713_webqsp.json",
                        help="right: 142, error: 306, pass: 0")
    ...
    for data in new_output_datas:
        # filter cot results
        if data["question"] in cot_question_list:
            continue
        try:
            if args.dataset=="webqsp":
        ...
        retrieve_results = str(data['reasoning_chains'])
        response = retrieve_results.replace("[","").replace("]","").strip()
